...
<Note>
  ...
  **Android:** No native UI is updated. An `Event.actionCallToggleMute` event is emitted through `onEvent` so your Dart code can respond and apply the mute state to your media stream.
</Note>
...
<Note>
  ...
  **Android:** Always returns `true` (the Android implementation does not track actual mute state).
</Note>
...
<Note>
  ...
  **Android:** No native UI is updated. An `Event.actionCallToggleHold` event is emitted through `onEvent` so your Dart code can respond and apply the hold state to your media stream.
</Note>
...
<Note>
**iOS:** Returns all active calls tracked by CallKit. Each entry contains `id` and `accepted` fields.

**Android:** Returns only the last recorded call. The result map includes an `isAccepted` field indicating whether the call was answered.
</Note>
