...
| `subtitle` | `String?` | Text displayed in the notification body. | — |
| `callbackText` | `String?` | Label for the callback action button. | — |
| `id` | `int?` | Internal Android notification ID. Auto-assigned if not set. | — |
| `count` | `int?` | Badge count shown on the notification (Android launcher support required). | — |

See [NotificationParams](/api/notification-params) for the full API reference.
